# 实现一个 TodoList - Vue2 Tutorials (二)
...
到此，基于 Vue 的 Todo List 就完成了。在该项目中，涉及到了组件化、双向绑定、自定义事件的触发与监听、计算属性等概念。而且最重要的不是完成这个 Todo List 的代码，而是从实现功能的过程中举一反三，通过简单的 demo 实现，去思考如何用 vue 开发一个更大更完整的项目。
